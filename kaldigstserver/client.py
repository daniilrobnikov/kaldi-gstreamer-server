...
class MyClient():
    ...
    async def run(self):
        self.ws = await websocket_connect(self.url, on_message_callback=self.received_message)
        if self.send_adaptation_state_filename is not None:
            print("Sending adaptation state from " +
        ...
        with self.audiofile as audiostream:
            while True:
                block = await self.ioloop.run_in_executor(executor, audiostream.read, int(self.byterate/4))
                if block == b"":
                    break
        ...
        if response['status'] == 0:
            ...
            if 'adaptation_state' in response:
                if self.save_adaptation_state_filename:
                    print("Saving adaptation state to " +
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Command line client for next-gen-kaldi-gstreamer-server')
    ...
    parser.add_argument('--content-type', default='',
                        help="Use the specified content type "
                        "(empty by default, for raw files the default is "
                        "audio/x-raw, layout=(string)interleaved, rate=(int)<rate>, "
                        "format=(string)S16LE, channels=(int)1")
    parser.add_argument('audiofile',
                        help="Audio file to be sent to the server", type=argparse.FileType('rb'))

    args = parser.parse_args()

    if not args.content_type and args.audiofile.name.endswith(".raw"):
        args.content_type = f"audio/x-raw, layout=(string)interleaved, rate=(int){args.rate//2}, format=(string)S16LE, channels=(int)1"

    query_params = urllib.parse.urlencode(
        [("content-type", args.content_type)])
    ws_url = f"{args.uri}?{query_params}"

    ws = MyClient(args.audiofile, ws_url, byterate=args.rate,
