...
class DecoderPipelineTests(unittest.TestCase):
    ...
    def test12345678(self):
        ...
        self.assertEqual(["üks", "kaks", "kolm", "neli", "<#s>",
                         "viis", "kuus", "seitse", "kaheksa", "<#s>"], self.words)
    ...
    def __testDecoder(self):
        ...
        def do_shit():
            decoder_pipeline.init_request(
                "test0", "audio/x-raw, layout=(string)interleaved, rate=(int)16000, format=(string)S16LE, channels=(int)1")
            f = open("test/data/1234-5678.raw", "rb")
            for block in iter(lambda: f.read(8000), ""):
                time.sleep(0.25)
        ...
        while not finished[0]:
            time.sleep(1)
        self.assertItemsEqual("see on esimene lause <#s> see on teine lause <#s>".split(
